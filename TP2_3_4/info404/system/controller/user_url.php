<?php
	...
	if (isset($_GET['page'])) {
		...
		case "subject":
			$page = "subject";
			if (isset($_GET['id']) && isset($_GET['id_message']) && isset($_GET['state']) && $_GET['state'] == 'del') {
				supprime_message($_GET['id_message']);
				header("location: ./?page=subject&id=".$_GET['id']);
			}
			elseif ((!isset($_GET['id']) || intval($_GET['id']) == 0) && (!isset($_GET['p']) || intval($_GET['p']) == 0)) {
